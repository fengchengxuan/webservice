...
newsa=true;aticalea=false;consulta=true;allordera=true;myevaluatea=true;fosa=true;videoadva=true;subjecta=true;deputynava=true">
    ...
    <div class="contentbox">
        ...
        <div class="inner" style="height: 620px">
            ...
            <div class="draftart-top">
                ...
                <input style="width: 269px" type="text"  placeholder="例如：网站数据" id="artKeyValue">
                <button class="key-search" id="KeySearch">搜索</button>
                <button class="search-title">标题搜索</button>
                <input style="width: 283px" type="text" placeholder="例如：网站建了多久才会有网站数据" id="artTilevalue">
...
<script type="text/javascript">
  ...
  $("#TitleSearch").click(function () {
     var titleValue=$("#artTilevalue").val();
     sreach(titleValue,"");
 });
    //关键词查询
    $("#KeySearch").click(function () {
        var keyValue=$("#artKeyValue").val();
        sreach("",keyValue)
    ...
    function sreach(titleValue,keyValue) {
      ...
      $("#systemId").val(0);
        $("#showId").empty();
        $.ajax({
            url : '${ctx}/Article/Search',
            type : 'POST',
            data : "artTitle="+titleValue+"&artKey="+keyValue,
            async:true,
            cache:false,
            dataType : 'json',
            success : function(data) {
                if(data!=null && data!=""){
                    ...
                    showArt(currentPage,list,totalNum);
...
//分页查询
    ...
    $(document).ready(function(){
        ...
        var artState=state;   //文章状态条件
        var artType=type;   //类型条件
        var systemId=system; //管理员类型
        $("#showId").empty();              //删除每一篇文章
         pagingSreach(artState,artType,systemId);

    });
    //开始异步
    function pagingSreach(state,type,system){   //根据条件取到相应的文章
        var artState=state;
        var artType=type;
        var systemId=system;
        $.ajax({
            ...
            type : 'POST',
            data : "artState="+artState+"&artType="+artType+"&systemId="+systemId,
            async:true,
            cache:false,
            dataType : 'json',
            success : function(data) {
                if(data!=null && data!=""){
                    ...
                    $("#totalNum").val(totalNum);
                    showArt(currentPage,list,totalNum);      //当前页 总文章 总页数
                    showpage(currentPage,totalNum);
                }
            }
        })
    ...
    //显示数据
    function showArt(pageData, list,total) {
        $("#showId").empty();
        ...
        if(currentPage!=totalNum&&currentPage<totalNum) {       //当前页不等于总页数
            ...
            for (i;i < strip ; i++) {
                ...
                var showRow = "<tr style=\"height: 36px;background-color: #F4F4F4 \">" +
                    ...
                    "&nbsp;|&nbsp;" +
                    "<a href=\"javascript:void(null)\" onclick=\"deleteArt("+i+")\"  ><span style=\"color: #EC2323\">删除</span></a>" +
                    " </td>" +
   ...
   $("#oldPage").click(function () {
       ...
       if(currentPage>1){
           ...
           showArt(nextPage,listArt,totalNum);
    ...
    $("#state").change(function () {
        ...
        reMovePage(currentPage);
        $("#currentPage").val(1);
        pagingSreach(value,artType,systemId);
    });
    //类型显示
     ...
     if($("#state").val()=="3"){
        ...
        if (confirm("是否要删除，亲")) {
            ...
            $.ajax({
                ...
                type : 'POST',
                data : "artTitle="+list[code].artTitle,
                async:true,
                cache:false,
                dataType : 'json',
                success : function(data) {
    ...
    function upDate(data){
        ...
        window.location.href = "${ctx}/admin/editarticles?code="+list[code].id;
    }
    //全刷新
    function reFreshAll(){
        $("#currentPage").val(1);
        var artState=$("#state").val();
        var artType=$("#artType").val();
        var systemId=$("#systemId").val();
        $("#showId").empty();
        var currentPage= $("#currentPage").val();
