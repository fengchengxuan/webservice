...
<div class="contener" style="overflow: hidden">
    ...
    <div class="contentbox">
        ...
        <div class="inner" style="height: 1270px">
            <div class="title">&nbsp;&nbsp;&nbsp;&nbsp;修改文章</div>
            <%--<label>--%>
            <%--<select name="" id="artType" class="revnewselect">--%>
                <%--<option value="0">选择文章发布类型</option>--%>
                <%--<option value="1">十二数据</option>--%>
            ...
            <div class="setnewsbox">
                <div class="newstitle">
                    ...
                    <input type="text" placeholder="" id="artTitle">
                    <div id="searchresult"></div>
                    <p class="end">文章标题字数不能超30个字，超出字数无法进行提交。</p>
                </div>
                <div class="newkeyw" style="margin: 30px 0 0 0">
                    <span class="head" style="letter-spacing: 1px">文章关键词</span>
                    <input type="text" placeholder="" id="artKey">
                    <p class="end">整篇文章最核心关键词及目标关键词、长尾关键词，字数15个字以内。</p>
                </div>
                <div class="abs" style="margin: 30px 0 0 0">
                    <span class="head" style="letter-spacing: 12px;position: relative;top:-50px">摘&nbsp;&nbsp;要</span>
                    <textarea class="abstext" placeholder="" id="artAbstract"></textarea>
                    <p class="end" style="position: relative;top:-76px">文章摘要字数不能超100个字，超出字数无法进行提交。</p>
                </div>
            </div>
            <div id="container" >
            </div>
...
<script type="text/javascript">
    ...
    $("#Submit").click(function(){
        ...
        var artKey=$("#artKey").val();
        var artAbstract=$("#artAbstract").val();
        var artType=$("#artType").val();
        var htmlText=ue.getContent();
        var newArtTitle="";
        var artTitle;
        if(flat==false){
            newArtTitle=$("#artTitle").val();
            if(oldArtTitle!=newArtTitle){
                artTitle=oldArtTitle;
            }else{
                artTitle=newArtTitle;
            }
        }else {
        ...
        $.ajax({
            url : '${ctx}/Article/Updata',
            type : 'POST',
            data : "htmlText="+htmlText+"&artTitle="+artTitle+"&artKey="+artKey+"&artAbstract="+artAbstract+"&artType="+artType+"&newArtTitle="+newArtTitle,
            async:true,
            cache:false,
            dataType : 'json',
            success : function(data){
    ...
    //显示数据
    function showArtData(data,i){
        var artType=data[i].artType;
        var artKey=data[i].artKey;
        var artAbstract=data[i].artAbstract;
        var htmlText=data[i].htmlText;
        $("#artType").val(artType);
        $("#artKey").val(artKey);
        $("#artAbstract").val(artAbstract);
    ...
    function emptyData() {
        ...
        $("#artKey").val(null);
        $("#artTitle").val(null);
        $("#artAbstract").val(null);
        ue.setContent('开始编辑文字');
        $("#artType").val(1);
    }


    $("#artTitle").keydown(function(){
        $("#artTitle").css("background-color","#c9ff8d");
    });

    //查数据
    $("#artTitle").keyup(function(){
        var artTitle=$("#artTitle").val();
        if(flat){
            if(artTitle!=null && artTitle!=""){
                $.ajax({
                    url : '${ctx}/Article/SearchTitle',
                    type : 'POST',
    ...
    function showTitle(data) {
        ...
        for(var i=0;i<data.length;i++){
            layer+="<tr class =\"line\"><td><input type=\"hidden\" value="+i+" >"+data[i].artTitle+"</td></tr>";
        }
        layer+="</table>";
        $("#searchresult").empty();
        ...
        $(".line").click(function () {
            ...
            $("#artTitle").val(data[num].artTitle);
            showArtData(data,num);
            tranSlate();
        })
    }
    //完全加载触发
