...
<style>
    ...
    #field_{
        margin-left: 0;width: 80px;height: 26px;resize:none;border: 1px solid #ccc;
    }
    ...
    <div class="contentbox">
        ...
        <div class="inner" style="height: auto;padding-bottom: 30px;">
            ...
            <select name="" id="productType" class="revnewselect" style="font-weight: 400">
                ...
                <option value="网站建设/程序/APP研发与定制">网站建设/程序/APP研发与定制</option>
                <option value="网络计划与制定">网络计划与制定</option>
                <option value="网络营销">网络营销</option>
                <option value="网络推广">网络推广</option>
                <option value="网络文案">网络文案</option>
                <option value="视频制作与推广">视频制作与推广</option>
                <option value="搜索优化">搜索优化</option>
                <option value="网络媒体">网络媒体</option>
                <option value="词条/百科">词条/百科</option>
                <option value="网络运营">网络运营</option>
                <option value="数据统计">数据统计</option>
                <option value="网络传媒及线上线下各类广告投放">网络传媒及线上线下各类广告投放</option>
            </select>
            </label>
            <div class="setnewsbox">
                ...
                <div class="fanan">
                    ...
                    服务标准
                        ...
                        <label>
                        <select style="margin-left: 12px;font-weight: 400;width: 68px" id="service">
                            <option>体验版</option>
                            <option>基础版</option>
                            <option>加强版</option>
                            <option>定制版</option>
                    ...
                    <div style="margin: -31px 0 0 345px;">
                         <span >
                    使用优惠券
                         </span>
                        <label>
                            <input type="checkbox" style="width: 18px;height: 18px;position: relative;top: 4px;">
                            <input id="preferential" type="text" style="width: 100px;">
                        </label>
                    </div>


                    <div style="margin-left: 12px;margin-bottom: 50px;">
                        <div>
                            <ul class="pay">
                            <li class="num">
                                <p style="float: left;width: 82px;position: relative;left: 13px">需求总数</p>
                                <input id="totalNum" type="text" style="width: 512px;margin: -2px 0 0 0;" placeholder="">
                            </li>
                            <li class="time" id="time">
                                <p style="float: left;width: 82px;position: relative;left: 13px">购买时长</p>
                    ...
                    赠送
                        ...
                        <input style="margin-left: 11px;width: 120px;height: 26px;resize:none;border: 1px solid #ccc;" type="text" name="mytext[]" id="field_1" value="" placeholder="">
                        <input style="margin-left: 0;width: 240px;height: 26px;resize:none;border: 1px solid #ccc;"  type="text" id="field1" placeholder="">

                        <input type="submit" value="添加" style="width: 80px;height: 40px"  id="AddMoreFileBox">
                        <input type="submit" value="查看" style="width: 80px;height: 40px"  id="AddMoreFile">
   ...
   $("#productTitle").keyup(function(){
    ...
    function showTitle(data) {
        ...
        $(".line").click(function () {
            ...
            $("#productTitle").val(data[num].proTitle);
            showProData(data,num);
            ue.setContent(data[num].htmlText);
        });
    ...
    function showProData(data,i){
        ...
        $("#productKey").val(proKey);
        $("#productAbstract").val(proAbstract);
        ue.setContent(htmlText);
        $("#searchresult").empty();
    }
    function emptyData(){//清空标题信息
        $("#productType").val("");
        $("#productKey").val(null);
        $("#productTitle").val(null);
        $("#productAbstract").val(null);
        ue.setContent('开始编辑文字');

    }
    //保存或提交
    function subOrPres(data) {
        var proTitle = $("#productTitle").val();   //产品标题
        var proKey = $("#productKey").val();      //产品关键字
        var proAbstract = $("#productAbstract").val();//产品摘要
        var proType = $("#productType").val();  //产品类型
        var htmlText = ue.getContent();                //产品描述  htmlText

        //------产品需求信息表
        var demand = $("#demand").val(); //需求
        var proService = $("#service").val();//服务标准
        var servicePrice = $("#servicePrice").val();//服务金额
        var hard_Configuration = $("#hard_Configuration").val();//硬性配置
        var soft_Configuration = $("#soft_Configuration").val();//软性配置
        var personnel = $("#personnel").val();//人员配置
        var time_Allocation = $("#time_Allocation").val();//时间配备
        var user_range = $("#user_range").val();//适合用户
        var dataInfo = $("#dataInfo").val();//数据统计
        var pro_manager = $("#pro_manager").val();//经理
        var preferential = $("#preferential").val();//优惠
        var technical_service = $("#technical_service").val();//技术服务
        var serviceGuarante = $("#serviceGuarante").val();//服务保障
        ...
        if (timeValue != "试客体验" && proService == "体验版") {
            alert("请选择试客体验")
            return
        }
        if (timeValue == "试客体验" && proService != "体验版") {
       ...
       $.ajax({
            url : '${ctx}/Product/add',
            type : 'POST',
            data : "htmlText="+htmlText+"&proTitle="+proTitle+"&proKey="+proKey+"&proAbstract="+proAbstract+"&proType="+proType+"&or="+data+
            "&demand="+demand+"&proService="+proService+"&servicePrice="+servicePrice+"&hard_Configuration="+hard_Configuration+"&soft_Configuration="+soft_Configuration+
            "&personnel="+personnel+"&time_Allocation="+time_Allocation+"&user_range="+user_range+"&dataInfo="+dataInfo+"&pro_manager="+pro_manager+
            "&technical_service="+technical_service+"&serviceGuarante="+serviceGuarante+"&preferential="+preferential+"&timeValue="+timeValue+"&num="+num+"&freeCode="+freeCode +"&freeText="+freeText,
            async:true,
            cache:false,
            dataType : 'json',
            success : function(data){
                if(data!=null){
                    alert(data[0]);
                    if(data[0]=="添加成功"){
                        window.location.href="${ctx}/admin/addproduct";
                }
                }
...
function givenFree(){
      freeCode = $("#field_1").val();
      freeText= $("#field1").val();
   for(var i=0;i<$(".givenFree").length;i++){
       if(i%2==0){
           if(i>0){
              if( $(".givenFree")[i-1].value==null ||$(".givenFree")[i-1].value){
                  alert("请输入正确的赠送信息")
              }
           }
           freeCode=freeCode+";"+$(".givenFree")[i].value;
       }else{
           freeText= freeText+";"+$(".givenFree")[i].value;
       }
   }
}
</script>
